  ...
  try {
    ...
    try {
      ...
      if (route) {
        ...
        // Let Google Maps render the route using its normal road-route styling.
        activeRideRouteLine =
      ...
      console.warn(
        "Active ride route drawing failed:",
        routeError
      );
...
async function updateActiveRideDriverToPickupRoute(
  ...
  try {
    ...
    if (
      ...
      if (route) {
        // Use Google's renderer for the driver-to-pickup road route,
        // rather than drawing a raw black polyline.
        activeRideDriverToPickupRouteLine =
          route.createPolylines({
            polylineOptions: {
              map: activeRideMap,
