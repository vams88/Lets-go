...
// ============================================================
// NEARBY ONLINE DRIVERS / RIDERS ON THE CUSTOMER BOOKING MAP
// Uses the existing driver_rider_locations availability feed.
// ============================================================
...
function getNearbyPartnerRoleVehicleFilter() {
  ...
  if (vehicleType === "bike") {
    return { role: "rider", vehicle_type: "bike" };
  }

  if (vehicleType === "auto") {
    return { role: "driver", vehicle_type: "auto" };
  }

  if (vehicleType === "car") {
    return { role: "driver", vehicle_type: "car" };
  }

  return null;
}

function getCustomerBookingMap() {
  // The booking map is stored in the module-scoped letsGoMap variable.
  return letsGoMap;
}

function nearbyPartnerMarkerOptions(vehicleType) {
  const label =
    vehicleType === "bike"
      ? "🏍️"
      : vehicleType === "auto"
...
async function loadNearbyPartners() {
  ...
  const map = getCustomerBookingMap();
  const filter = getNearbyPartnerRoleVehicleFilter();

  if (!map || !filter || !window.google?.maps) {
  ...
  try {
    // Only show a partner while their availability GPS is reasonably fresh.
    ...
    if (locationError) {
      console.warn(
        "Nearby partner locations unavailable:",
    ...
    if (profileError) {
      console.warn(
        "Nearby partner profiles unavailable:",
...
// nearby-partner-start
document.addEventListener("DOMContentLoaded", () => {
  setTimeout(
    startNearbyPartnerUpdates,
    1200
  );
});

// Refresh immediately when the customer switches Bike / Auto / Car.
