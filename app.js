...
async function cancelBooking(bookingId) {
  ...
  if (!cancelledBooking) {
    alert(
      "This ride can no longer be cancelled. It may already have started or finished."
    );
    return;
  }
        ...
        handler:
          async response => {
            ...
            try {
              ...
              await showCustomerRatingPrompt(paidBooking);
    ...
    <div class="card">
      ...
      ${
        ...
        (
          ...
          ? `
            <p><strong>Payment: Cash pending — please pay ₹${Number(booking.fare_amount || 0).toFixed(2).replace(/\.00$/, "")} to your Driver/Rider.</strong></p>
          `
          : ""
      }
...
async function loadRides() {
  ...
  // After loading completed rides, offer the customer a rating for the
  // most recent completed ride that has been paid and not yet rated.
  // This also works after cash is marked as received by the driver/rider.
  const unratedPaidRide = recentRides.find(booking =>
    booking &&
    booking.status === "completed" &&
    booking.payment_status === "paid" &&
    booking.driver_id
  ...
  if (unratedPaidRide) {
    setTimeout(() => {
      void showCustomerRatingPrompt(unratedPaidRide);
    }, 350);
  }
...
// ============================================================
// CUSTOMER MY RATING DATA
// Loads ratings submitted by the currently logged-in customer.
// ============================================================
async function loadCustomerMyRating() {
  try {
    const user = currentAuthUser || (await supabase.auth.getUser()).data?.user;

    if (!user?.id) return;

    // Customer My Rating = ratings RECEIVED by this customer.
    // Ratings the customer gives to a driver/rider belong in that
    // driver's/rider's profile rating, not in the customer's rating.
    ...
    if (error) {
      console.error("Customer rating load failed:", error);
      return;
    }
    ...
    window.letsGoCustomerRating = {
      average,
      count: total
    };

    const averageEl = document.getElementById("lgCustomerAverageRating");
    const starsEl = document.getElementById("lgCustomerRatingStars");
    const countEl = document.getElementById("lgCustomerRatingCount");

    if (averageEl) averageEl.textContent = average ? average.toFixed(1) : "—";
    ...
    console.error("Unexpected customer rating error:", error);
  }
}

window.loadCustomerMyRating = loadCustomerMyRating;



// ============================================================
// CUSTOMER POST-PAYMENT RATING
// ============================================================

async function hasCustomerRatedBooking(bookingId, userId) {
  const { data, error } = await supabase
    .from("ratings")
    .select("id")
  ...
  if (error) {
    console.error("Rating existence check failed:", error);
    return false;
  }
...
function closeCustomerRatingPrompt() {
  document.getElementById("lgCustomerRatingPrompt")?.remove();
}

async function submitCustomerRideRating(booking, rating, feedback) {
  const user = currentAuthUser || (await supabase.auth.getUser()).data?.user;

  if (!user?.id || !booking?.id || !booking?.driver_id) {
    alert("Unable to identify this ride partner. Please try again later.");
    return;
  }

  const alreadyRated = await hasCustomerRatedBooking(booking.id, user.id);
  if (alreadyRated) {
    closeCustomerRatingPrompt();
    return;
  }
  ...
  const { error } = await supabase.from("ratings").insert({
    ...
    rater_id: user.id,
    rated_user_id: booking.driver_id,
  ...
  closeCustomerRatingPrompt();
  await loadCustomerMyRating();
  alert("Thank you! Your rating has been saved.");
}

async function showCustomerRatingPrompt(booking) {
  if (!booking?.id || booking.status !== "completed" || booking.payment_status !== "paid") return;

  const user = currentAuthUser || (await supabase.auth.getUser()).data?.user;
  if (!user?.id || !booking.driver_id) return;

  if (await hasCustomerRatedBooking(booking.id, user.id)) return;
  if (document.getElementById("lgCustomerRatingPrompt")) return;

  const wrapper = document.createElement("div");
  wrapper.id = "lgCustomerRatingPrompt";
  wrapper.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;z-index:99999;padding:18px;";
  wrapper.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:22px;max-width:380px;width:100%;box-shadow:0 10px 35px rgba(0,0,0,.25);">
      <h3 style="margin:0 0 8px;">Rate your ride</h3>
      <p style="margin:0 0 16px;color:#555;">How was your experience with your driver/rider?</p>
      <div id="lgRatingChoices" style="display:flex;gap:8px;justify-content:center;margin:12px 0 18px;">
        ${[1,2,3,4,5].map(n => `<button type="button" data-rating="${n}" style="font-size:28px;border:0;background:none;cursor:pointer;color:#bbb;">☆</button>`).join("")}
      </div>
      <textarea id="lgRatingFeedback" placeholder="Optional feedback" rows="3" style="width:100%;box-sizing:border-box;border:1px solid #ddd;border-radius:10px;padding:10px;resize:vertical;"></textarea>
      <button id="lgSubmitRating" type="button" disabled style="width:100%;margin-top:14px;padding:12px;border:0;border-radius:10px;background:#999;color:#fff;cursor:pointer;">Submit Rating</button>
      <button id="lgSkipRating" type="button" style="width:100%;margin-top:8px;padding:10px;border:0;background:transparent;color:#666;cursor:pointer;">Maybe later</button>
    </div>
  `;
  ...
  wrapper.querySelectorAll("[data-rating]").forEach(button => {
    button.addEventListener("click", () => {
      selectedRating = Number(button.dataset.rating);
      wrapper.querySelectorAll("[data-rating]").forEach(item => {
        item.textContent = Number(item.dataset.rating) <= selectedRating ? "★" : "☆";
        item.style.color = Number(item.dataset.rating) <= selectedRating ? "#f5a623" : "#bbb";
      });
      const submit = document.getElementById("lgSubmitRating");
      submit.disabled = false;
      submit.style.background = "#111";
    });
  });

  document.getElementById("lgSubmitRating").addEventListener("click", async () => {
    if (!selectedRating) return;
    const button = document.getElementById("lgSubmitRating");
    button.disabled = true;
    button.textContent = "Saving...";
    await submitCustomerRideRating(
      booking,
      selectedRating,
      document.getElementById("lgRatingFeedback").value
    );
  });

  document.getElementById("lgSkipRating").addEventListener("click", closeCustomerRatingPrompt);
}

window.showCustomerRatingPrompt = showCustomerRatingPrompt;
