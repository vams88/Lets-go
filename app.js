...
bookBtn?.addEventListener(
  ...
  async () => {
    ...
    // The route-selection screen can hide the original text fields. In that
    // state the authoritative pickup/destination values are the stored map
    // coordinates. Do not reject a valid route just because the hidden input
    // is empty.
    let pickupValue =
      pickup?.value.trim() || "";
    ...
    // If a location was selected by GPS/map but its display text is empty,
    // create a safe fallback label from the stored coordinates.
    const locationToCoords = (value) => {
    ...
    const destinationCoords =
      locationToCoords(destinationLocation);

    if (!pickupValue && pickupCoords) {
      pickupValue =
        `Current location (${pickupCoords.lat.toFixed(5)}, ${pickupCoords.lng.toFixed(5)})`;
    }

    if (!destinationValue && destinationCoords) {
      destinationValue =
        `Selected destination (${destinationCoords.lat.toFixed(5)}, ${destinationCoords.lng.toFixed(5)})`;
    }
    ...
    if (
      (!pickupValue && !pickupCoords) ||
      (!destinationValue && !destinationCoords)
    ) {

      if (status) {

        status.textContent =
          "Please select a valid pickup and destination.";
