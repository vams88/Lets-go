...
function setCustomerPaymentMethod(method) {
  ...
  [lgCashPaymentBtn, lgUpiPaymentBtn]
    .filter(Boolean)
    .forEach(button => {
      const active =
  ...
  if (status && window.__letsGoCurrentDistanceKm) {
    const distance = Number(window.__letsGoCurrentDistanceKm);
    ...
    if (fare !== null) {
      status.textContent =
        `₹${fare} • ${distance.toFixed(2)} km • ${selectedCustomerPaymentMethod === "upi" ? "UPI" : "Cash"}`;
    }
  }
}

lgCashPaymentBtn?.addEventListener("click", () =>
  setCustomerPaymentMethod("cash")
);

lgUpiPaymentBtn?.addEventListener("click", () =>
