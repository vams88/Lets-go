...
// ============================================================
// NEARBY ONLINE DRIVERS / RIDERS
// ============================================================
// This uses the existing partner availability GPS table.
// It does NOT modify the customer's current-location flow.
...
function getNearbyServiceFilter() {
...
function getNearbyPartnerMarkerIcon(vehicleType) {
  const symbol =
    vehicleType === "bike"
      ? "🏍️"
      : vehicleType === "auto"
...
async function loadNearbyPartnersOnBookingMap() {
  if (nearbyPartnersRequestInFlight) {
    return;
  }

  if (!letsGoMap || !window.google?.maps || !supabase) {
    return;
  }

  const filter = getNearbyServiceFilter();
  ...
  try {
    // A partner location older than two minutes is considered stale.
    ...
    if (locationError) {
      console.warn(
        "Could not load nearby partner locations:",
    ...
    if (profileError) {
      console.warn(
        "Could not load nearby partner profiles:",
    ...
    (locations || []).forEach((location) => {
      const profile =
        profileById.get(location.driver_id);
      ...
      const marker =
        new google.maps.Marker({
          ...
          title:
            `${profile.full_name || "Available partner"} • ` +
            `${profile.vehicle_type || filter.vehicle_type}`,
          icon: getNearbyPartnerMarkerIcon(
            filter.vehicle_type
          ),
          zIndex: 50
        });
...
function startNearbyPartnerMapUpdates() {
  if (nearbyPartnersTimer) {
    clearInterval(nearbyPartnersTimer);
  }

  void loadNearbyPartnersOnBookingMap();
...
function stopNearbyPartnerMapUpdates() {
  if (nearbyPartnersTimer) {
    clearInterval(nearbyPartnersTimer);
  }
...
// nearby-partner-service-change
document.addEventListener("change", (event) => {
  if (event.target?.id === "rideType") {
    setTimeout(() => {
      void loadNearbyPartnersOnBookingMap();
    }, 100);
  }
