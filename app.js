...
// ===== Nearby Driver/Rider markers on the customer booking map =====
let nearbyPartnerMarkers = [];
let nearbyPartnersTimer = null;
let nearbyPartnersRequestInFlight = false;

function clearNearbyPartnerMarkers() {
  if (!nearbyPartnerMarkers.length) return;
  nearbyPartnerMarkers.forEach(marker => {
    try { marker.setMap(null); } catch (_) {}
  });
  nearbyPartnerMarkers = [];
}

function getSelectedServiceVehicleType() {
  ...
  if (service.includes("bike")) return "bike";
  if (service.includes("auto")) return "auto";
  if (service.includes("cab") || service.includes("car")) return "car";
  return service;
}

function getNearbyPartnerRoleVehicleFilter() {
  const vehicleType = getSelectedServiceVehicleType();
  if (vehicleType === "bike") return { role: "rider", vehicle_type: "bike" };
  if (vehicleType === "auto") return { role: "driver", vehicle_type: "auto" };
  if (vehicleType === "car") return { role: "driver", vehicle_type: "car" };
  return null;
}

function getCustomerMapObject() {
  return window.map || window.customerMap || window.bookingMap || null;
}

function makeNearbyPartnerIcon(vehicleType) {
  const emoji = vehicleType === "bike" ? "🏍️" : vehicleType === "auto" ? "🛺" : "🚗";
  return {
    text: emoji,
    fontSize: "24px",
    fontFamily: "Arial",
...
async function loadNearbyPartners() {
  if (nearbyPartnersRequestInFlight) return;

  const map = getCustomerMapObject();
  const filter = getNearbyPartnerRoleVehicleFilter();
  if (!map || !filter) {
    clearNearbyPartnerMarkers();
    return;
  }

  nearbyPartnersRequestInFlight = true;
  ...
  try {
    ...
    if (error) {
      console.warn("Could not load nearby partner locations:", error.message);
      clearNearbyPartnerMarkers();
      return;
    }

    const ids = (data || []).map(x => x.driver_id).filter(Boolean);
    if (!ids.length) {
      clearNearbyPartnerMarkers();
      return;
    }
    ...
    const { data: profiles, error: profileError } = await supabaseClient
      .from("profiles")
      .select("id, full_name, role, vehicle_type, is_online")
      .in("id", ids)
      .eq("is_online", true)
      .eq("role", filter.role)
      .eq("vehicle_type", filter.vehicle_type);

    if (profileError) {
      console.warn("Could not load nearby partner profiles:", profileError.message);
      clearNearbyPartnerMarkers();
      return;
    }
    ...
    (data || []).forEach(loc => {
      if (!allowed.has(loc.driver_id)) return;

      const lat = Number(loc.latitude);
      const lng = Number(loc.longitude);
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) return;

      const p = profileById.get(loc.driver_id);
      const marker = new google.maps.Marker({
        map,
        position: { lat, lng },
        title: `${p?.vehicle_type || filter.vehicle_type} • ${p?.full_name || "Available partner"}`,
        icon: makeNearbyPartnerIcon(filter.vehicle_type),
        zIndex: 20
      });

      nearbyPartnerMarkers.push(marker);
    });
  } catch (err) {
    console.warn("Nearby partner map error:", err);
    clearNearbyPartnerMarkers();
  } finally {
    nearbyPartnersRequestInFlight = false;
  }
}

function startNearbyPartnerUpdates() {
  if (nearbyPartnersTimer) clearInterval(nearbyPartnersTimer);
  loadNearbyPartners();
  nearbyPartnersTimer = setInterval(loadNearbyPartners, 15000);
}

function stopNearbyPartnerUpdates() {
  if (nearbyPartnersTimer) clearInterval(nearbyPartnersTimer);
  nearbyPartnersTimer = null;
  clearNearbyPartnerMarkers();
}
  ...
  pickupInput.addEventListener(
    ...
    () => {
      ...
      if (!pickupFieldUserInteracted) {
        // The app is expected to fill the current location automatically.
        // Merely focusing an empty field must not show a fake "Current
        // location" Places suggestion. If the customer wants another
        // pickup, typing will set pickupFieldUserInteracted=true and open
        // normal Google Places suggestions.
        clearLocationSuggestions(pickupInput);
        return;
      }
...
async function updateAuthUI(authUser = undefined) {
  ...
  profileLogoutBtn.textContent = "Log Out";
      ...
      // Shared-site role routing: authenticated partners/admins must not
      // remain in the customer landing application. The driver/rider app
      // and admin app use the same Supabase project and auth session.
      const currentPath = window.location.pathname;
      const isDriverRiderApp = currentPath.startsWith("/driver-rider");
      const isAdminApp = currentPath.startsWith("/admin");

      if (role === "driver" || role === "rider") {
        if (!isDriverRiderApp) {
          window.location.replace("/driver-rider/");
          return;
        }
      } else if (role === "admin") {
        if (!isAdminApp) {
      ...
      if (role === "customer") {
        ...
        // Automatically use the customer's device location as pickup when
        // the booking screen opens. The customer should not have to tap the
        // current-location button. If they want a different pickup, they can
        // simply type into the field and normal Google Places suggestions
        // will appear.
        if (
          pickup &&
          pickup.value.trim() === "" &&
          (!lgActiveRidePanel || lgActiveRidePanel.classList.contains("hidden"))
        ) {
          void useCurrentLocationForPickup();
        }
  ...
  try {
    ...
    const razorpay =
      new window.Razorpay({
        ...
        // Keep the customer's selected payment method in our booking,
        // but do not force Razorpay Checkout into a UPI-only block.
        // Forcing UPI-only can produce "No appropriate payment method found"
        // when UPI is not currently available for the merchant/order.
...
// Customer Help & Support: show the latest ride before raising a last-ride issue.
window.letsGoShowLastRideSupport = async function(container){
  if(!container) return;
  container.innerHTML = "Loading your latest ride...";
  try {
    const user = currentAuthUser || (await supabase.auth.getUser()).data?.user;
    if(!user) throw new Error("Please log in again.");
    const {data: ride, error} = await supabase.from("bookings").select("id,service,pickup_location,destination,booking_date,booking_time,status,fare_amount,payment_method,created_at").eq("user_id", user.id).order("created_at", {ascending:false}).limit(1).maybeSingle();
    if(error) throw error;
    if(!ride){ container.innerHTML = `<p>No previous ride was found.</p><a class="btn" href="https://mail.google.com/mail/?view=cm&fs=1&to=support%40letsgoapp.in&su=Last%20Ride%20Help">Contact Support</a>`; return; }
    const esc = v => String(v ?? "—").replace(/[&<>"]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));
    const details = `Service: ${esc(ride.service)}<br>From: ${esc(ride.pickup_location)}<br>To: ${esc(ride.destination)}<br>Status: ${esc(ride.status)}<br>Fare: ₹${esc(ride.fare_amount ?? "—")}`;
    const subject = encodeURIComponent("Help with my last ride - " + ride.id);
    container.innerHTML = `<div style="padding:12px;border:1px solid var(--line);border-radius:12px;margin-bottom:12px;"><strong>Latest ride</strong><div style="margin-top:8px;line-height:1.7;">${details}</div></div><p>Describe the issue in your email.</p><a class="btn" href="https://mail.google.com/mail/?view=cm&fs=1&to=support%40letsgoapp.in&su=${subject}">Contact Support</a>`;
  } catch(e) { container.innerHTML = `<p>Could not load your latest ride.</p><a class="btn" href="https://mail.google.com/mail/?view=cm&fs=1&to=support%40letsgoapp.in&su=Last%20Ride%20Help">Contact Support</a>`; }
};
...
let customerProfileSnapshot = {
  ...
  createdAt: "", emergencyName: "", emergencyPhone: ""
};

function setCustomerProfileStatus(message = "") {
  if (lgCustomerProfileStatus) lgCustomerProfileStatus.textContent = message;
}

function profileField(id, value, type="text", readonly=false, options=[]) {
  const el=document.getElementById(id); if(!el) return;
  if(!el.dataset.originalLabel) el.dataset.originalLabel=el.textContent;
  const input=document.createElement(type==="select"?"select":"input");
  input.id=id+"Input"; input.className="lg-customer-profile-input"; input.value=value||"";
  if(type!=="select") input.type=type; if(readonly) input.readOnly=true;
  if(type==="select") { options.forEach(x=>{const o=document.createElement("option");o.value=x;o.textContent=x;input.appendChild(o)}); input.value=value||""; }
  el.replaceWith(input);
}
function displayProfileField(id,value){ const old=document.getElementById(id+"Input"); if(old){const d=document.createElement("div");d.id=id;d.className="lg-customer-profile-value";d.textContent=value||"—";old.replaceWith(d);} }
function setCustomerProfileEditing(isEditing) {
  if(isEditing){
    profileField("lgCustomerProfileName",customerProfileSnapshot.fullName);
    profileField("lgCustomerProfilePhone",customerProfileSnapshot.phone,"tel",true);
    profileField("lgCustomerProfileEmail",customerProfileSnapshot.email,"email",true);
    profileField("lgCustomerProfileDob",customerProfileSnapshot.dob,"date");
    profileField("lgCustomerProfileGender",customerProfileSnapshot.gender,"select",false,["","Male","Female","Other","Prefer not to say"]);
    profileField("lgCustomerEmergencyName",customerProfileSnapshot.emergencyName);
    profileField("lgCustomerEmergencyPhone",customerProfileSnapshot.emergencyPhone,"tel");
    lgCustomerProfileEditBtn?.classList.add("hidden"); lgCustomerProfileSaveBtn?.classList.remove("hidden"); lgCustomerProfileCancelBtn?.classList.remove("hidden");
    setCustomerProfileStatus("Edit your details and save changes.");
    return;
  }
  displayProfileField("lgCustomerProfileName",customerProfileSnapshot.fullName); displayProfileField("lgCustomerProfilePhone",customerProfileSnapshot.phone); displayProfileField("lgCustomerProfileEmail",customerProfileSnapshot.email); displayProfileField("lgCustomerProfileDob",customerProfileSnapshot.dob); displayProfileField("lgCustomerProfileGender",customerProfileSnapshot.gender); displayProfileField("lgCustomerEmergencyName",customerProfileSnapshot.emergencyName); displayProfileField("lgCustomerEmergencyPhone",customerProfileSnapshot.emergencyPhone);
  lgCustomerProfileEditBtn?.classList.remove("hidden"); lgCustomerProfileSaveBtn?.classList.add("hidden"); lgCustomerProfileCancelBtn?.classList.add("hidden");
}
...
function renderCustomerProfile(data = {}) {
  ...
  customerProfileSnapshot = { ...customerProfileSnapshot, fullName, phone, email, dob:String(data.dob||""), gender:String(data.gender||""), createdAt:String(data.createdAt||""), emergencyName:String(data.emergencyName||""), emergencyPhone:String(data.emergencyPhone||"") };
  ...
  if (lgCustomerProfileEmail) lgCustomerProfileEmail.textContent = email || "—";
  [ ["lgCustomerProfileDob",data.dob], ["lgCustomerProfileGender",data.gender], ["lgCustomerProfileCreatedAt",data.createdAt], ["lgCustomerEmergencyName",data.emergencyName], ["lgCustomerEmergencyPhone",data.emergencyPhone] ].forEach(([id,v])=>{const e=document.getElementById(id);if(e)e.textContent=v||"—";});

  // Keep the main Profile hero card synchronized with the authenticated
  // profile details. Previously this card remained stuck on placeholders
  // because only the detailed profile page was being updated.
  const heroName = document.getElementById("profileFullName");
  const heroPhone = document.getElementById("profilePhone");
  const heroEmail = document.getElementById("profileEmail");
  if (heroName) heroName.textContent = fullName || "Your Profile";
  if (heroPhone) heroPhone.textContent = phone || "Phone number";
  if (heroEmail) heroEmail.textContent = email || "Account details";

  window.dispatchEvent(new CustomEvent("letsgo:profileData", { detail: {
    fullName, phone, email, rating: data.rating || ""
  }}));
...
async function loadCustomerProfilePage() {
  ...
  const {
    ...
    .from("profiles")
    .select("full_name, phone, date_of_birth, gender, emergency_contact_name, emergency_contact_phone, created_at")
    .eq("id", user.id)
    .maybeSingle();
  ...
  renderCustomerProfile({
    ...
    dob: profile?.date_of_birth || "", gender: profile?.gender || "",
...
lgCustomerProfileSaveBtn?.addEventListener(
  ...
  async event => {
    ...
    const phoneInput = document.getElementById("lgCustomerProfilePhoneInput");
    const dobInput=document.getElementById("lgCustomerProfileDobInput");
    const genderInput=document.getElementById("lgCustomerProfileGenderInput");
    const emergencyNameInput=document.getElementById("lgCustomerEmergencyNameInput");
    const emergencyPhoneInput=document.getElementById("lgCustomerEmergencyPhoneInput");
    ...
    const { error: updateError } =
      await supabase
        ...
        .update({
          ...
          emergency_contact_name: emergencyNameInput?.value.trim() || null,
          emergency_contact_phone: emergencyPhoneInput?.value.trim() || null
        })
        .eq("id", user.id);
    ...
    renderCustomerProfile({
      fullName,
      phone, dob:dobInput?.value||"", gender:genderInput?.value||"",
      createdAt:customerProfileSnapshot.createdAt, emergencyName:emergencyNameInput?.value.trim()||"", emergencyPhone:emergencyPhoneInput?.value.trim()||"",
      email: user.email ||
        user.phone ||
...
// Start/refresh nearby availability markers when the customer changes vehicle/service.
document.addEventListener("change", (event) => {
  ...
  if (
    ...
    setTimeout(loadNearbyPartners, 50);
  }
});


// nearby_partner_dom_start
document.addEventListener("DOMContentLoaded", () => {
  setTimeout(startNearbyPartnerUpdates, 1000);
});


// nearby_partner_unload
window.addEventListener("beforeunload", stopNearbyPartnerUpdates);


// Unified multi-role mode support
async function loadUnifiedUserRoles() {
  if (!currentUser?.id) return [];
  const { data } = await supabase.from("user_roles").select("role, approved").eq("user_id", currentUser.id).eq("approved", true);
  const roles = new Set((data || []).map(x => x.role));
  roles.add("customer");
  return [...roles];
}
async function switchUnifiedMode(mode) {
  const roles = await loadUnifiedUserRoles();
  if (mode !== "customer" && !roles.includes(mode)) { alert("This mode is not approved for your account."); return; }
  localStorage.setItem("letsgo_active_mode", mode);
  if (mode === "customer") { window.location.href = "/"; return; }
  if (mode === "driver" || mode === "rider") { window.location.href = "/driver-rider/"; return; }
  if (mode === "admin") { window.location.href = "/admin/"; return; }
}

// Customer profile portal: rides and support actions
(function installCustomerProfilePortalHelpers(){
  const esc = value => String(value ?? "").replace(/[&<>'"]/g, ch => ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[ch]));
  const formatDate = value => value ? new Date(value).toLocaleString("en-IN", {dateStyle:"medium", timeStyle:"short"}) : "—";
  window.letsGoLoadProfileRides = async container => {
    if (!container) return;
    const user = currentAuthUser || currentUser;
    if (!user?.id) { container.innerHTML = "<p>Please log in to view your rides.</p>"; return; }
    const {data, error} = await supabase.from("bookings").select("*").eq("user_id", user.id).order("created_at", {ascending:false}).limit(7);
    if (error) { container.innerHTML = `<p>Could not load rides: ${esc(error.message)}</p>`; return; }
    if (!data?.length) { container.innerHTML = "<p>No rides found yet.</p>"; return; }
    container.innerHTML = `<div class="profile-ride-list">${data.map(r => `<div class="profile-ride-card"><strong>${esc(r.service || "Ride")} · ${esc(r.status || "—")}</strong><div>${esc(r.pickup_location || "—")} → ${esc(r.destination || "—")}</div><div>${esc(formatDate(r.created_at || r.booking_date))} · Fare: ₹${esc(r.fare_amount ?? "—")}</div><div>Payment: ${esc(r.payment_method || "—")}</div></div>`).join("")}</div>`;
  };
  window.letsGoOpenLastRideSupport = async container => {
    const user = currentAuthUser || currentUser;
    if (!container) return;
    if (!user?.id) { container.innerHTML = "<p>Please log in first.</p>"; return; }
    const {data, error} = await supabase.from("bookings").select("*").eq("user_id", user.id).order("created_at", {ascending:false}).limit(1).maybeSingle();
    if (error || !data) {
      container.innerHTML = '<p>No previous ride was found.</p><button type="button" class="support-email-button" data-support-direct="other">Contact support</button>';
      container.querySelector("[data-support-direct]")?.addEventListener("click", () => window.letsGoOpenSupportRequest?.("other", container));
      return;
    }
    container.innerHTML = `<h3 style="margin:0 0 10px;">Your latest ride</h3><div class="profile-ride-card"><strong>${esc(data.service || "Ride")} · ${esc(data.status || "—")}</strong><div>${esc(data.pickup_location || "—")} → ${esc(data.destination || "—")}</div><div>${esc(formatDate(data.created_at || data.booking_date))} · Fare: ₹${esc(data.fare_amount ?? "—")}</div><div>Ride ID: ${esc(data.id || "—")}</div></div><p style="margin-top:14px;">Select the problem you want to report:</p><div class="support-choice-list"><button type="button" data-last-ride-issue="payment">Payment issue</button><button type="button" data-last-ride-issue="partner">Driver/Rider issue</button><button type="button" data-last-ride-issue="cancellation">Cancellation issue</button><button type="button" data-last-ride-issue="other">Other issue</button></div><button type="button" class="support-back-button" data-support-back>Back to support categories</button>`;
    container.querySelectorAll("[data-last-ride-issue]").forEach(btn => btn.addEventListener("click", () => window.letsGoOpenSupportRequest?.(btn.dataset.lastRideIssue, container, data)));
    container.querySelector("[data-support-back]")?.addEventListener("click", () => window.letsGoRenderSupportCategories?.(container));
  };
  window.letsGoRenderSupportCategories = container => {
    if (!container) return;
    container.innerHTML = `<p>What do you need help with?</p><div class="support-choice-list"><button type="button" data-support-category="last_ride">Help with my last ride</button><button type="button" data-support-category="payment">Payment issue</button><button type="button" data-support-category="partner">Driver/Rider issue</button><button type="button" data-support-category="cancellation">Cancellation issue</button><button type="button" data-support-category="account">Account issue</button><button type="button" data-support-category="other">Other issue</button></div>`;
    container.querySelectorAll("[data-support-category]").forEach(btn => btn.addEventListener("click", () => {
      const type = btn.dataset.supportCategory;
      if (type === "last_ride") window.letsGoOpenLastRideSupport?.(container);
      else window.letsGoOpenSupportRequest?.(type, container);
    }));
  };
  window.letsGoOpenSupportRequest = (type, container, ride=null) => {
    const labels = {payment:"Payment issue",partner:"Driver/Rider issue",cancellation:"Cancellation issue",account:"Account issue",other:"Other issue"};
    const label = labels[type] || "Support request";
    if (!container) return;
    container.innerHTML = `<p>You selected: <strong>${esc(label)}</strong></p>${ride ? `<div class="profile-ride-card"><strong>${esc(ride.service || "Ride")} · ${esc(ride.status || "—")}</strong><div>${esc(ride.pickup_location || "—")} → ${esc(ride.destination || "—")}</div><div>Ride ID: ${esc(ride.id || "—")}</div></div>` : ""}<label style="display:block;margin:14px 0 6px;font-weight:600;">Describe your issue</label><textarea data-support-message rows="5" placeholder="Tell us what happened and how we can help..."></textarea><button type="button" class="support-email-button" data-send-support>Send support request</button><button type="button" class="support-back-button" data-support-back>Back</button>`;
    container.querySelector("[data-send-support]")?.addEventListener("click", () => {
      const message = container.querySelector("[data-support-message]")?.value.trim();
      if (!message) { alert("Please describe your issue before sending."); return; }
      const subject = encodeURIComponent(`Let's Go Support - ${label}`);
      const body = encodeURIComponent(`Issue: ${label}\n${ride ? `Ride ID: ${ride.id}\nService: ${ride.service || ""}\nStatus: ${ride.status || ""}\n` : ""}\nCustomer message:\n${message}`);
      const supportEmail = "support@letsgoapp.in";
      const composeUrl = `https://mail.google.com/mail/u/0/?view=cm&fs=1&tf=1&to=${encodeURIComponent(supportEmail)}&su=${subject}&body=${body}`;
      // Android preview/WebView containers commonly redirect Gmail compose URLs to
      // the inbox. Do not silently pretend the message was addressed or sent.
      const isWebView = /; wv\)|\bwv\b|Acode|SPCK|HtmlViewer|WebView/i.test(navigator.userAgent || "");
      if (isWebView) {
        container.innerHTML = `<div class="profile-ride-card"><strong>Your support request is ready</strong><p style="margin:8px 0 4px;">Send it to:</p><p style="font-weight:700;word-break:break-all;">${supportEmail}</p><p style="margin:8px 0 0;">Your preview app cannot open Gmail compose directly. Copy the address and paste it into a new email.</p></div><div class="support-choice-list"><button type="button" data-copy-support-email>Copy support email</button><button type="button" data-open-gmail-compose>Try Gmail compose</button></div><button type="button" class="support-back-button" data-support-back>Back</button>`;
        container.querySelector("[data-copy-support-email]")?.addEventListener("click", async () => {
          try { await navigator.clipboard.writeText(supportEmail); alert("Support email copied: " + supportEmail); }
          catch (_) { window.prompt("Copy this support email:", supportEmail); }
        });
        container.querySelector("[data-open-gmail-compose]")?.addEventListener("click", () => window.open(composeUrl, "_blank"));
        container.querySelector("[data-support-back]")?.addEventListener("click", () => ride ? window.letsGoOpenLastRideSupport?.(container) : window.letsGoRenderSupportCategories?.(container));
        return;
      }
