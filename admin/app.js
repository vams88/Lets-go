...
function setAdminTab(tab){
  ...
  applications?.classList.toggle("hidden", showBookings || showPartners || showAnalytics || showHistoryRequests);
  ...
  applicationsTab?.classList.toggle("active", !showBookings && !showPartners && !showAnalytics && !showHistoryRequests);
    ...
    return `
      <div class="app-row ${pending ? "application-pending" : ""}">
        ...
        <div class="meta">
          ...
          <div><b>Aadhaar</b>${app.aadhaar_file_path ? `Submitted${app.aadhaar_number ? ` (•••• ${escapeHtml(String(app.aadhaar_number).slice(-4))})` : ""}` : "Not submitted"}</div>
          <div><b>PAN</b>${app.pan_file_path ? `Submitted${app.pan_number ? ` (••••${escapeHtml(String(app.pan_number).slice(-4))})` : ""}` : "Not submitted"}</div>
...
async function openApplication(id, list) {
  ...
  async function signedDocumentHtml(path, title){
    if(!path) return `<div class="notice">${escapeHtml(title)} was not submitted.</div>`;
    const { data, error } = await supabase.storage
      .from("driver-documents")
      .createSignedUrl(path, 600);
    if(error || !data?.signedUrl){
      return `<div class="notice error">Could not open the private ${escapeHtml(title)} document.</div>`;
    }
    const lower = path.toLowerCase();
    if(lower.endsWith(".jpg") || lower.endsWith(".jpeg") || lower.endsWith(".png") || lower.endsWith(".webp")){
      return `<div class="kyc-doc-block"><h3>${escapeHtml(title)}</h3><img class="doc" src="${escapeHtml(data.signedUrl)}" alt="${escapeHtml(title)} document"></div>`;
    }
    return `<div class="kyc-doc-block"><h3>${escapeHtml(title)}</h3><div class="pdfbox"><p>${escapeHtml(title)} document is available as a private file.</p><a href="${escapeHtml(data.signedUrl)}" target="_blank" rel="noopener">Open ${escapeHtml(title)} Document</a></div></div>`;
  }

  const [licenseHtml, aadhaarHtml, panHtml] = await Promise.all([
    signedDocumentHtml(currentApplication.license_file_path, "Driving Licence"),
    signedDocumentHtml(currentApplication.aadhaar_file_path, "Aadhaar"),
    signedDocumentHtml(currentApplication.pan_file_path, "PAN")
  ]);

  const maskedAadhaar = currentApplication.aadhaar_number
    ? `•••• •••• ${escapeHtml(String(currentApplication.aadhaar_number).slice(-4))}`
    : "Not submitted";
  const maskedPan = currentApplication.pan_number
    ? `••••••${escapeHtml(String(currentApplication.pan_number).slice(-4))}`
    : "Not submitted";
    ...
    <div class="detail-grid">
      <div>
        <div class="meta">
          ...
          <div><b>Aadhaar</b>${maskedAadhaar}</div>
          <div><b>PAN</b>${maskedPan}</div>
          <div><b>KYC Documents</b>${currentApplication.aadhaar_file_path && currentApplication.pan_file_path ? "All submitted" : "Incomplete"}</div>
...
async function reviewApplication(approved) {
  ...
  if (approved) {
    const missing = [];
    if (!currentApplication.license_file_path) missing.push("Driving Licence");
    if (!currentApplication.aadhaar_file_path) missing.push("Aadhaar");
    if (!currentApplication.pan_file_path) missing.push("PAN");
    if (missing.length) {
      showMessage(detailMessage, `Cannot approve yet. Missing: ${missing.join(", ")}.`);
      return;
    }
  }
