...
async function getSession() {
  ...
  // Validate the browser session against Supabase and refresh it when needed.
  ...
  if (session?.access_token) {
    ...
    if (verified.data?.user) session.user = verified.data.user;
  }
  return session;
}
...
async function showDashboard() {
  ...
  if(activeTab === "payoutsTab") await loadAdminPayouts();
    if(activeTab === "financialTab") await loadFinancialOverview();
  } catch (error) {
    console.error("Admin dashboard load failed:", error);
    showMessage(dashboardMessage, error?.message || "Unable to load Admin dashboard.");
  } finally {
...
function formatBookingSchedule(booking){
  ...
  const time = String(booking?.booking_time || "").trim();
  ...
  return d.toLocaleString("en-IN", {
    ...
    hour: time ? "2-digit" : undefined, minute: time ? "2-digit" : undefined, hour12:true
  }) + " IST";
}

function bookingSearchText(booking, customer, partner){
  return [
    booking?.id, booking?.service, booking?.pickup_location, booking?.destination,
    booking?.phone, booking?.payment_method, booking?.payment_status,
    customer?.full_name, customer?.phone, partner?.full_name, partner?.phone,
    partner?.vehicle_registration_number
...
function renderAdminBookings(){
  ...
  const rows = statusRows.filter(b => {
    const customer = profileMap.get(b.user_id);
    const partner = profileMap.get(b.driver_id);
    const payment = String(b.payment_status || "").trim().toLowerCase();
    const paymentMatches = paymentFilter === "all"
      || (paymentFilter === "paid" && payment === "paid")
      || (paymentFilter === "unpaid" && payment && payment !== "paid")
      || (paymentFilter === "not_recorded" && !payment);
    const searchMatches = !search || bookingSearchText(b, customer, partner).includes(search);
    return paymentMatches && searchMatches;
  });

  const summary = document.getElementById("bookingSummaryGrid");
  if(summary){
    const countStatus = status => adminBookings.filter(b => String(b.status || "").toLowerCase() === status).length;
    const active = adminBookings.filter(b => ["accepted","in_progress"].includes(String(b.status || "").toLowerCase())).length;
    const unpaidCompleted = adminBookings.filter(b => String(b.status || "").toLowerCase() === "completed" && String(b.payment_status || "").toLowerCase() !== "paid").length;
    const attention = adminBookings.filter(b => String(b.status || "").toLowerCase() === "pending" && !b.driver_id).length;
    summary.innerHTML = [
      ["Total bookings", adminBookings.length],
      ["Pending", countStatus("pending")],
      ["Active rides", active],
      ["Completed unpaid", unpaidCompleted],
      ["Needs attention", attention]
    ].map(([label,value]) => `<div class="booking-summary-card"><b>${escapeHtml(label)}</b><div class="booking-summary-value">${escapeHtml(value)}</div></div>`).join("");
  }
    ...
    return `
      <div class="booking-card${needsAttention ? " needs-attention" : ""}">
        ...
        <div class="status-grid">
          ...
          <div><b>Pickup</b><br>${escapeHtml(b.pickup_location || "—")}</div>
          <div><b>Destination</b><br>${escapeHtml(b.destination || "—")}</div>
          <div><b>Booking created</b><br>${escapeHtml(formatAdminIST(b.created_at))}</div>
          <div><b>Scheduled date & time</b><br>${escapeHtml(formatBookingSchedule(b))}</div>
          <div><b>Customer phone</b><br>${escapeHtml(b.phone || customer?.phone || "—")}</div>
          <div><b>Distance</b><br>${b.distance_km != null ? escapeHtml(String(b.distance_km)) + " km" : "—"}</div>
          <div><b>Fare</b><br>${b.fare_amount != null ? "₹" + escapeHtml(String(b.fare_amount)) : "—"}</div>
          <div><b>Payment</b><br>${escapeHtml(b.payment_status || "—")}${b.payment_method ? " • " + escapeHtml(b.payment_method) : ""}</div>
          <div><b>Razorpay payment ID</b><br>${escapeHtml(b.razorpay_payment_id || "—")}</div>
...
async function loadAdminPartners(){
  ...
  adminPartnerEarnings = new Map();
  adminPartnerKyc = new Map();
  ...
  if(ids.length){
    const { data: ledger, error: ledgerError } = await supabase
      ...
      .select("partner_id,gross_fare,platform_fee,partner_earnings,payout_status")
      .in("partner_id", ids);
    if(ledgerError){
      console.warn("Could not load partner earnings:", ledgerError);
    }else{
      for(const id of ids){
        adminPartnerEarnings.set(id, { gross:0, fee:0, earnings:0, pending:0, paid:0 });
      }
      for(const row of ledger || []){
        const e = adminPartnerEarnings.get(row.partner_id);
        if(!e) continue;
        e.gross += Number(row.gross_fare) || 0;
        e.fee += Number(row.platform_fee) || 0;
        e.earnings += Number(row.partner_earnings) || 0;
        const status = String(row.payout_status || "").toLowerCase();
        if(status === "pending") e.pending += Number(row.partner_earnings) || 0;
        if(status === "paid") e.paid += Number(row.partner_earnings) || 0;
      }
    }

    const { data: applications, error: appError } = await supabase
      .from("driver_rider_applications")
      .select("user_id,status,license_number,license_file_path,aadhaar_file_path,pan_file_path,vehicle_registration_number,reviewed_at")
      .in("user_id", ids)
      .order("created_at", { ascending:false });
    if(appError){
      console.warn("Could not load partner KYC status:", appError);
    }else{
      for(const row of applications || []){
        if(!adminPartnerKyc.has(row.user_id)) adminPartnerKyc.set(row.user_id, row);
      }
    }
  }
...
function renderPartnerSummary(partners, period){
  ...
  box.innerHTML = `
    ...
    <div class="partner-summary-card"><div class="label">Pending partner payouts</div><div class="value">₹${partners.reduce((sum,p)=>(sum+(adminPartnerEarnings.get(p.id)?.pending||0)),0).toFixed(2)}</div></div>
  `;
}
...
function renderAdminPartners(){
    ...
    return `
      <div class="partner-card">
        ...
        <div class="status-grid">
          ...
          <div><b>Total earnings</b><br>₹${escapeHtml((adminPartnerEarnings.get(p.id)?.earnings || 0).toFixed(2))}</div>
          <div><b>Pending payout</b><br>₹${escapeHtml((adminPartnerEarnings.get(p.id)?.pending || 0).toFixed(2))}</div>
          <div><b>Paid earnings</b><br>₹${escapeHtml((adminPartnerEarnings.get(p.id)?.paid || 0).toFixed(2))}</div>
          <div><b>KYC status</b><br>${escapeHtml(adminPartnerKyc.get(p.id)?.status || "Not found")}</div>
        ...
        <div class="partner-actions">
          <button class="secondary" data-partner-details="${escapeHtml(p.id)}">View Profile</button>
          <button class="secondary" data-partner-earnings="${escapeHtml(p.id)}">View Earnings</button>
          <button class="secondary" data-partner-current="${escapeHtml(p.id)}">View Current Ride</button>
          <button class="secondary" data-partner-kyc="${escapeHtml(p.id)}">View KYC</button>
          ${online && !busy
  ...
  const partnerById = id => adminPartners.find(x => x.id === id);
  const showPartnerNotice = (title, body) => {
    const box = document.getElementById("partnersList");
    if(!box) return;
    const existing = document.getElementById("partnerActionNotice");
    if(existing) existing.remove();
    const div = document.createElement("div");
    div.id = "partnerActionNotice";
    div.className = "card";
    div.style.marginBottom = "12px";
    div.innerHTML = `<h3>${escapeHtml(title)}</h3><div class="notice">${body}</div>`;
    box.prepend(div);
  };

  list.querySelectorAll("[data-partner-details]").forEach(btn => btn.addEventListener("click", () => {
    const p = partnerById(btn.dataset.partnerDetails); if(!p) return;
    showPartnerNotice("Partner Profile", `<b>${escapeHtml(p.full_name || "Unnamed")}</b><br>Phone: ${escapeHtml(p.phone || "—")}<br>Role: ${escapeHtml(p.role || "—")}<br>Vehicle: ${escapeHtml(adminVehicleLabel(p.vehicle_type))}<br>Registration: ${escapeHtml(p.vehicle_registration_number || "—")}<br>Status: ${p.is_online ? "Online" : "Offline"}`);
  }));
  list.querySelectorAll("[data-partner-earnings]").forEach(btn => btn.addEventListener("click", () => {
    const p = partnerById(btn.dataset.partnerEarnings); const e = adminPartnerEarnings.get(btn.dataset.partnerEarnings) || {gross:0,fee:0,earnings:0,pending:0,paid:0}; if(!p) return;
    showPartnerNotice("Partner Earnings", `Gross fares: <b>₹${e.gross.toFixed(2)}</b><br>Let's Go fees: <b>₹${e.fee.toFixed(2)}</b><br>Partner earnings: <b>₹${e.earnings.toFixed(2)}</b><br>Pending payout: <b>₹${e.pending.toFixed(2)}</b><br>Paid earnings: <b>₹${e.paid.toFixed(2)}</b>`);
  }));
  list.querySelectorAll("[data-partner-current]").forEach(btn => btn.addEventListener("click", () => {
    const id=btn.dataset.partnerCurrent; const active=adminPartnerBookings.find(b=>b.driver_id===id); const p=partnerById(id); if(!p) return;
    showPartnerNotice("Current Ride", active ? `<b>${escapeHtml(adminStatusLabel(active.status))}</b><br>Booking: ${escapeHtml(active.id)}<br>${escapeHtml(active.pickup_location || "—")} → ${escapeHtml(active.destination || "—")}<br>Fare: ₹${(Number(active.fare_amount)||0).toFixed(2)}` : `No active ride for ${escapeHtml(p.full_name || "this partner")}.`);
  }));
  list.querySelectorAll("[data-partner-kyc]").forEach(btn => btn.addEventListener("click", () => {
    const id=btn.dataset.partnerKyc; const k=adminPartnerKyc.get(id); const p=partnerById(id); if(!p) return;
    const complete=!!(k?.license_file_path && k?.aadhaar_file_path && k?.pan_file_path && k?.license_number);
    showPartnerNotice("KYC Status", `Application status: <b>${escapeHtml(k?.status || "Not found")}</b><br>Licence document: ${k?.license_file_path ? "Present" : "Missing"}<br>Aadhaar document: ${k?.aadhaar_file_path ? "Present" : "Missing"}<br>PAN document: ${k?.pan_file_path ? "Present" : "Missing"}<br>Licence number: ${k?.license_number ? "Present" : "Missing"}<br><b>Document completeness: ${complete ? "Complete" : "Incomplete"}</b>`);
  }));
...
function setAdminTab(tab){
  ...
  const showFinancial = tab === "financial";

  applications?.classList.toggle("hidden", showBookings || showPartners || showAnalytics || showHistoryRequests || showPayouts || showFinancial);
  ...
  financial?.classList.toggle("hidden", !showFinancial);
  ...
  financialTab?.classList.toggle("active", showFinancial);
  if(showFinancial) void loadFinancialOverview();
...
async function loadFinancialOverview(){
  ...
  const msg = document.getElementById("financialMessage");
  const summary = document.getElementById("financialSummary");
  const payments = document.getElementById("financialPayments");
  showMessage(msg, "Loading financial overview...");
  const {data, error} = await supabase.rpc("admin_get_financial_overview", {p_period: period});
  if(error){ showMessage(msg, error.message || "Could not load financial overview.", "error"); return; }
  const n=(v)=>Number(v||0); const money=(v)=>formatMoney(n(v));
  summary.innerHTML = [
    ["Total bookings",n(data.total_bookings)], ["Completed rides",n(data.completed_rides)], ["Cancelled rides",n(data.cancelled_rides)], ["Active rides",n(data.active_rides)],
    ["Gross completed fare",money(data.gross_completed_fare)], ["Let's Go platform fees",money(data.total_platform_fee)], ["Partner earnings",money(data.total_partner_earnings)], ["Pending payouts",money(data.pending_partner_payouts)], ["Paid payouts",money(data.paid_partner_payouts)], ["Completed unpaid",n(data.unpaid_completed_count)]
  ].map(x=>`<div class="overview-stat"><b>${x[0]}</b><div class="overview-value">${x[1]}</div></div>`).join("");
  payments.innerHTML=`<div class="overview-grid"><div class="overview-stat"><b>UPI completed</b><div class="overview-value">${n(data.upi_payments)}</div></div><div class="overview-stat"><b>Cash completed</b><div class="overview-value">${n(data.cash_payments)}</div></div><div class="overview-stat"><b>Pay Later completed</b><div class="overview-value">${n(data.pay_later_payments)}</div></div><div class="overview-stat"><b>Paid booking records</b><div class="overview-value">${n(data.paid_booking_count)}</div></div></div>`;
  showMessage(msg, "Financial overview updated.", "success");
}
...
document.getElementById("financialTab")?.addEventListener("click", () => setAdminTab("financial"));
document.getElementById("financialPeriodFilter")?.addEventListener("change", loadFinancialOverview);
document.getElementById("financialRefreshBtn")?.addEventListener("click", loadFinancialOverview);
...
document.getElementById("bookingPaymentFilter")?.addEventListener("change", renderAdminBookings);
document.getElementById("bookingSearch")?.addEventListener("input", renderAdminBookings);
document.getElementById("bookingsRefreshBtn")?.addEventListener("click", () => loadAdminBookings());
...
supabase.auth.onAuthStateChange(async (event, session) => {
  ...
  if (event === "SIGNED_IN" && session.user?.id !== lastSessionUserId) {
    try {
...
(async function init() {
  try {
    const session = await getSession();
    initialized = true;
    if (session?.user && await requireAdmin()) {
      lastSessionUserId = session.user.id;
      await showDashboard();
    } else showLogin();
  } catch (error) {
