...
async function loadPartnerEarningsLedger(){
  ...
  const { data: settings, error: settingsError } = await supabase
    .from("letgo_financial_settings")
    .select("platform_fee_percent")
    .eq("id", 1)
    .maybeSingle();

  if(settingsError){
    console.error("Financial settings error:", settingsError);
    if(history) history.innerHTML = '<div class="status bad">Could not load earnings ledger: ' + escapeHTML(settingsError.message || "Unknown error") + '</div>';
    return;
  }

  partnerLedgerCommissionRate = Number(settings?.platform_fee_percent);
