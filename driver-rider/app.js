...
async function cancelAcceptedBooking(bookingId){
  ...
  const confirmed = window.confirm("Cancel this accepted ride? The customer will be notified that the ride was cancelled.");
  if(!confirmed) return;
  ...
  try{
    ...
    stopLiveLocationTracking();
    setStatus($("dashboardStatus"), "Ride cancelled successfully.", "ok");
    await loadDriverBookings();
  }catch(err){
    console.error("Driver cancellation error:", err);
    setStatus($("dashboardStatus"), err.message || "Could not cancel the ride.", "bad");
    await loadDriverBookings();
  }
}
