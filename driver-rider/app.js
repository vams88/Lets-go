...
async function refreshDriverMapFromBookings(){
  const mapCard = $("driverMapCard");
  const requestHint = $("rideRequestHint");
  const currentlyOnline = Boolean($("onlineToggle")?.checked);
  ...
  if(!activeBooking){
    ...
    if(requestHint){
      requestHint.textContent = currentlyOnline
  ...
  if(requestHint) requestHint.textContent = "Your assigned booking is shown below.";
