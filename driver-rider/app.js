...
function formatBookingDateTime(booking){
  // bookings.created_at is a timestamptz and Supabase returns it as an
  // absolute timestamp. Always display ride history in India Standard Time.
  ...
  // Fallback for older rows that may not have a usable created_at value.
    ...
    return `
      <div class="ride-card">
        ...
        <div class="ride-meta">
          ...
          <span>${b.fare_amount != null ? "₹" + escapeHTML(String(b.fare_amount)) : ""}</span>
        </div>
        ${actions}
      </div>`;
  }).join("");
    ...
    return `
      <div class="ride-card">
        ...
        <div class="ride-meta">
          ...
          ${b.fare_amount != null ? `<span>₹${escapeHTML(String(b.fare_amount))}</span>` : ""}
        </div>
      </div>`;
  }).join("");
}
