...
async function publishAvailabilityLocation(position){
  ...
  try{
    ...
    if(error) console.warn("Could not publish availability location.", error);
  }catch(err){
    console.warn("Could not publish availability location.", err);
  }finally{
...
function startAvailabilityLocationSharing(){
  ...
  if(!currentUser || !toggle?.checked || !navigator.geolocation) return;

  const send = () => navigator.geolocation.getCurrentPosition(
    pos => publishAvailabilityLocation({
      lat: pos.coords.latitude,
      lng: pos.coords.longitude
    }),
    err => console.warn("Availability location error.", err),
